    ...
    return (
        <div className="space-y-8 pb-20 animate-fade-in text-white">
            ...
            {activeTournaments.length === 0 && finishedTournaments.length === 0 ? (
                <div className="text-center py-20 bg-slate-800/50 rounded-[2.5rem] border-2 border-dashed border-slate-700">
                    <div className="w-20 h-20 bg-slate-800 rounded-full flex items-center justify-center mx-auto mb-6 text-slate-600">
                        <Trophy size={40}/>
                    </div>
                    <h3 className="text-xl font-bold text-white">No hay torneos</h3>
                    ...
                    <button 
                        onClick={() => navigate('/setup')}
                        className="px-8 py-3 bg-white text-slate-900 font-black rounded-xl text-sm hover:bg-slate-100 transition-colors"
                    >
                        CREAR PRIMER TORNEO
                    </button>
                </div>
                ...
                <div className="space-y-8">
                            ...
                            {activeTournaments.map(t => {
                                ...
                                return (
                                    <div 
                                        ...
                                        className="bg-slate-800/80 backdrop-blur-sm rounded-3xl border border-slate-700 shadow-xl overflow-hidden cursor-pointer hover:border-[#575AF9]/50 transition-all group relative"
                                    >
                                        <div className="p-6">
                                            <div className="flex justify-between items-start mb-4">
                                                <div className="w-3/4">
                                                    <div className="flex items-center gap-2 mb-2">
                                                        <span className={`w-2.5 h-2.5 rounded-full ${isSetup ? 'bg-orange-500 shadow-[0_0_8px_rgba(249,115,22,0.5)]' : 'bg-emerald-500 animate-pulse shadow-[0_0_8px_rgba(16,185,129,0.5)]'}`}></span>
                                                        <span className={`text-[10px] font-black uppercase tracking-widest ${isSetup ? 'text-orange-400' : 'text-emerald-400'}`}>
                                                ...
                                                <div className="flex flex-col items-center justify-center bg-slate-900 rounded-2xl p-3 min-w-[70px] border border-slate-700/50 shadow-inner">
                                                    <span className="text-2xl font-black text-white leading-none">{t.playerCount || 0}</span>
                                                    <span className="text-[9px] font-black uppercase text-slate-500 mt-1">Parejas</span>
                                                </div>
                                            </div>
                                            ...
                                            <div className="mt-6 pt-4 border-t border-slate-700 flex items-center justify-between">
                                                <span style={{ 
                                                    color: '#fff', 
                                                    backgroundColor: isSetup ? '#334155' : '#0f172a',
                                                ...
                                                {isSetup ? (
                                                    ...
                                                    <span className="text-xs font-black text-emerald-400 flex items-center gap-1 group-hover:underline">
                    ...
                    {finishedTournaments.length > 0 && (
                        <div>
                            <h3 className="text-sm font-bold text-slate-500 uppercase tracking-wider mb-4 px-2">Finalizados Recientemente</h3>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 opacity-70 hover:opacity-100 transition-opacity">
                                {finishedTournaments.map(t => (
                                    <div key={t.id} className="bg-slate-900 border border-slate-800 p-4 rounded-2xl flex justify-between items-center">
                                        <div>
                                            <div className="font-bold text-slate-300">{t.title}</div>
                                            <div className="text-xs text-slate-500">{new Date(t.date).toLocaleDateString()}</div>
                                        </div>
                                        <span className="text-xs font-bold bg-slate-800 px-3 py-1 rounded-lg text-slate-500">Finalizado</span>
                                    </div>
                                ))}
                            </div>
                        </div>
