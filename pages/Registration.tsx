...
const Registration: React.FC = () => {
  const { state, addPlayerToDB, createPairInDB, updatePairDB, deletePairDB, startTournamentDB } = useTournament();
  ...
  const PlayerSelector = ({ 
      ...
      const handleCreatePlayer = async () => {
          ...
          if(newId) {
              onSelect(newId);
              setNewPlayer({ name: '', nickname: '', categories: [], saveRecord: false });
              setTab('search'); 
          }
      };
              ...
              {selectedId ? (
                  <div className="flex items-center justify-between bg-white p-3 rounded-lg border border-emerald-200 shadow-sm animate-fade-in">
                      <div className="flex items-center gap-3">
                          <div className="w-8 h-8 bg-emerald-100 rounded-full flex items-center justify-center text-emerald-600 border border-emerald-200">
                              <User size={16} />
                          </div>
                          <div>
                              <div className="font-bold text-slate-800 text-sm">{selectedPlayer?.name}</div>
                              {selectedPlayer?.nickname && <div className="text-xs text-slate-500 italic">"{selectedPlayer.nickname}"</div>}
                          </div>
                      </div>
                  ...
                  <>
                      ...
                      <div className="flex bg-white p-1 rounded-lg border border-slate-200 mb-3 shadow-sm">
                          <button 
                            ...
                            className={`flex-1 py-2 text-xs font-bold rounded-md transition-all flex items-center justify-center gap-2 ${tab === 'search' ? 'bg-slate-100 text-slate-900 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                          >
                              <Search size={14}/> Buscar
                          </button>
                          <button 
                            onClick={() => setTab('new')} 
                            className={`flex-1 py-2 text-xs font-bold rounded-md transition-all flex items-center justify-center gap-2 ${tab === 'new' ? 'bg-slate-100 text-slate-900 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                          >
                              <Plus size={14}/> Nuevo
                          </button>
                      </div>
                      ...
                      {tab === 'search' ? (
                          <div className="animate-fade-in">
                              <input 
                                ...
                                className="w-full p-3 text-sm bg-white border border-slate-300 rounded-lg mb-2 focus:border-blue-500 outline-none text-slate-800 placeholder:text-slate-400 shadow-inner"
                                autoFocus
                              />
                              <div className="max-h-40 overflow-y-auto space-y-1 pr-1 custom-scrollbar">
                                  {filteredPlayers.slice(0, 50).map(p => (
                                      <button 
                                        ...
                                        className="w-full text-left p-2 hover:bg-blue-50 rounded flex items-center justify-between text-sm text-slate-700 transition-colors border border-transparent hover:border-blue-100"
                                      >
                                          <span className="font-medium">{p.name}</span>
                                          <div className="flex items-center gap-2">
                                              {p.categories?.[0] && <span className="text-[10px] bg-slate-100 px-1.5 py-0.5 rounded text-slate-500">{p.categories[0]}</span>}
                                              {p.nickname && <span className="text-xs text-slate-400 italic">"{p.nickname}"</span>}
                                          </div>
                                      </button>
                                  ))}
                                  {filteredPlayers.length === 0 && <p className="text-xs text-center text-slate-400 py-4 italic">No se encontraron jugadores.</p>}
                              </div>
                          </div>
                      ) : (
                          <div className="space-y-3 animate-fade-in">
                              <input 
                                ...
                                className="w-full p-3 text-sm bg-white border border-slate-300 rounded-lg outline-none focus:border-emerald-500 text-slate-800 placeholder:text-slate-400"
                                autoFocus
                              />
                              <input 
                              ...
                              <div className="flex flex-wrap gap-1.5">
                                  {TOURNAMENT_CATEGORIES.map(c => (
                                      <button 
                                        key={c} 
                                        onClick={() => toggleNewCat(c)}
                                        className={`px-2.5 py-1.5 text-[10px] font-bold rounded-md border transition-all ${newPlayer.categories.includes(c) ? 'bg-emerald-500 text-white border-emerald-600 shadow-sm' : 'bg-white text-slate-500 border-slate-200 hover:border-emerald-200'}`}
                                      >
                                          {c}
                                      </button>
                                  ))}
                              ...
                              <div 
                                onClick={() => setNewPlayer(p => ({...p, saveRecord: !p.saveRecord}))}
                                className={`flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition-all ${newPlayer.saveRecord ? 'bg-emerald-50 border-emerald-200' : 'bg-white border-slate-200 hover:bg-slate-50'}`}
                              >
                                  <div className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${newPlayer.saveRecord ? 'bg-emerald-500 border-emerald-500' : 'bg-white border-slate-300'}`}>
                                      {newPlayer.saveRecord && <Check size={14} className="text-white" strokeWidth={3} />}
                                  </div>
                              ...
                              <button 
                                ...
                                className="w-full py-3 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-sm font-bold shadow-md transition-colors flex items-center justify-center gap-2"
                              >
                                  <Check size={16}/> Usar Jugador
                              </button>
                          </div>
                      )}
                  </>
                    ...
                    return (
                        <div key={pair.id} className="bg-white p-4 rounded-xl flex items-center justify-between border border-slate-200 shadow-sm hover:shadow-md transition-shadow">
                            <div className="flex items-center gap-4 overflow-hidden w-full">
                                <span className="bg-slate-100 w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold shrink-0 text-slate-500 border border-slate-200">
                                    {idx + 1}
                                </span>
                                <div className="flex flex-col w-full">
                                    <div className="text-base font-bold text-slate-800 truncate">{p1?.name}</div>
                                    <div className="text-base font-bold text-slate-800 truncate text-slate-500">& {p2?.name}</div>
                                </div>
                            </div>
    ...
    <div className="space-y-6 pb-20">
      ...
      {viewMode === 'menu' && (
          <>
            <button 
                onClick={() => {
                    setIsEditingPairId(null); setSelectedP1(''); setSelectedP2('');
                    setViewMode('pair-form');
                }}
                className="w-full bg-white hover:bg-emerald-50 border-2 border-emerald-100 hover:border-emerald-300 p-6 rounded-2xl flex flex-col items-center justify-center gap-2 transition-all shadow-sm active:scale-95 group"
            >
            ...
            {activePairs.length >= 2 && state.status === 'setup' && (
                <div className="fixed bottom-20 left-4 right-4 z-40">
                    <button 
                        ...
                        className="w-full bg-gradient-to-r from-emerald-600 to-teal-600 text-white py-5 rounded-2xl font-bold shadow-xl text-2xl animate-pulse active:scale-95 transition-transform"
                    >
                        EMPEZAR TORNEO
                    </button>
                </div>
      ...
      {viewMode === 'pair-form' && (
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-lg animate-slide-up">
              ...
              <div className="flex gap-3 mt-8 pt-4 border-t border-slate-100">
                  ...
                  <button onClick={handleSavePair} className="flex-1 py-4 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-bold shadow-lg flex items-center justify-center gap-2 transition-colors active:scale-95">
                      <Save size={20} /> Guardar Pareja
                  </button>
              </div>
          </div>
      )}
