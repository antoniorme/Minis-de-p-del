    ...
    <div className={`min-h-screen transition-colors duration-500 flex flex-col md:flex-row ${getBodyBackground()} ${isDarkMode || isContextMode ? 'text-slate-50' : 'text-slate-900'}`}>
      ...
      <div className={`flex-1 flex flex-col ${isSidebarCollapsed ? 'md:pl-20' : 'md:pl-64'} min-h-screen transition-all duration-300`}>
          ...
          <div className="md:hidden pt-2 px-2 pb-2 sticky top-0 z-40 transition-colors duration-500">
              <header className={`px-4 py-3 flex justify-between items-center border rounded-2xl shadow-sm transition-all duration-500 ${isContextMode ? 'bg-indigo-500 border-indigo-300 shadow-indigo-500/20' : isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
